...
@php
    ...
    $nowMonth = (int) date('n');
@endphp
<tr wire:key="row-{{ $student->id }}" data-sid="{{ $student->id }}">
    <td class="sticky-col col-checkbox"><input type="checkbox" class="row-check" aria-label="{{ $student->name }}"></td>
    <td class="sticky-col col-id">{{ $student->external_id ?? $student->id }}</td>
    <td class="sticky-col col-name">
        <a href="#" class="row-link" data-act="pay" data-m="{{ $nowMonth }}" title="{{ __('actions.add_payment') }} ({{ __('filters.month') }} {{ $nowMonth }})">{{ $student->name }}</a>
    </td>
    <td class="col-phone">{{ $student->phone_primary_e164 ?: '—' }}</td>
    <td>
        @if ($row['siblings'] > 0)
            <span class="sibling-badge" data-act="family" title="{{ __('actions.show_family') }}">👨‍👧 {{ $row['siblings'] + 1 }}</span>
        @else
            <span class="sibling-badge-solo" data-act="family" title="{{ __('actions.show_family') }}">👤</span>
        @endif
    </td>
    @foreach ($built['cells'] as $m => $c)
        <td class="cell-month {{ $c['class'] }}" data-act="pay" data-m="{{ $m }}" role="button" tabindex="0" title="{{ $months[$m] }} — {{ $c['label'] }} · {{ __('actions.add_payment') }}"><div class="cell-content"><span class="amount">{{ $c['display'] }}</span><span class="method-icon">{{ $c['methodIcon'] }}</span><span class="cell-hint" aria-hidden="true">+</span></div></td>
    @endforeach
